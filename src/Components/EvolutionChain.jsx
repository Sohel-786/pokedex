    ...
    function handleEvoChain(chain){
       ...
       list.push(
        <li className="w-[20.2%] relative my-[2em] float-left">
            <img src={temp?.img} alt={temp?.name} className="rounded-[50%] bg-[#616161] shadow-evo border-[5px] border-white mx-auto max-w-[150px] w-full block" />
            <h3 className="capitalize text-white text-center text-[125%] leading-[125%] w-full my-[15px]" style={{
                fontFamily : 'sans-serif'
            }}>{temp?.name + " "  +temp?.number}</h3>
            <ul>
